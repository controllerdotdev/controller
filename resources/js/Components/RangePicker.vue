...
const setAllTime = () => {
    range.value = {
        group: "month",
        start: dayjs("2018-01-10").startOf("day").utc().format("YYYY-MM-DD"),
        end: dayjs().endOf("day").utc().format("YYYY-MM-DD"),
        timezone: date.getUserTimezone(),
    };
};
    ...
    <div class="relative">
        ...
        >
            <template #default="{ togglePopover, inputValue }">
                <div
                    class="flex items-center justify-between space-x-2 bg-zinc-100 dark:bg-zinc-900 px-3 py-2 rounded text-sm text-zinc-900 dark:text-zinc-300 dark:hover:text-zinc-100 hover:bg-zinc-50 dark:hover:bg-zinc-900 cursor-pointer border border-zinc-200 dark:border-zinc-700 whitespace-normal text-nowrap"
                    @click="() => togglePopover()"
                >
